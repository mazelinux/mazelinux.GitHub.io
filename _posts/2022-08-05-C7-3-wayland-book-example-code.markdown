...
就目前所学知识，我们已经可以编写一个 Wayland client 应用程序，并让它在屏幕上显示一些内
容。下面的代码是一个完整的 Wayland client 应用程序，它创建了一个 XDG toplevel 窗口，并
在其上显示一个 640x480 的正方形。请使用如下方法进行编译:
...
然后通过 `./client` 命令来查看它的运行情况，或者使用 `WAYLAND_DEBUG=1 ./client` ，这样
会包含许多有用的调试信息。完美！在以后的章节中，我们将基于此 client 代码进行修改，所以请将
此代码藏在一个安全的地方。
